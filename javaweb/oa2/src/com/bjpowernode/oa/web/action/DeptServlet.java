...
// 模糊匹配
// 只要请求路径是一“/dept”开始的，都走这个 Servlet。
// @WebServlet("/dept/*")
public class DeptServlet extends HttpServlet {
    ...
    private void doEdit(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            if (rs.next()) {
                String dname = rs.getString("dname");
                String location = rs.getString("location"); // 参数“location”是 sql 语句查询结果的列名。
                // 输出动态网页。
    ...
    private void doDel(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (count == 1) {
            // 删除成功
            // 仍然跳转到部门列表页面
            // 部门列表页面的显示需要执行另一个 Servlet，怎么办？转发。
            // request.getRequestDispatcher("/dept/list").forward(request, response);
            response.sendRedirect(request.getContextPath() + "/dept/list");
        } else {
            // 删除失败。
    ...
    private void doModify(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (count == 1) {
            // 更新成功
            // 跳转到部门列表页面（部门列表页面是通过 Java 程序动态生成 ，所以还需要再次执行另一个 Servlet）
            // request.getRequestDispatcher("/dept/list").forward(request, response);
