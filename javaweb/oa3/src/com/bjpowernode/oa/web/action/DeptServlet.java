...
public class DeptServlet extends HttpServlet {
    // 模板方法
    // 重写 service 方法（并没有重写 doGet 或者 doPost）
    ...
    protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        */
        ...
        // 获取 session（这个 session 是不需要新建的）
        // 只是获取当前的 session，获取不到则返回 null
        /*
        ...
        if (session != null && session.getAttribute("username") != null) {
            ...
            response.sendRedirect(request.getContextPath() + "/index.jsp"); // 访问 web 站点的根即可，自动找到欢迎页面。
        }
    ...
    /*
     * 连接数据库，查询所有的部门信息，将部门信息收集好，然后跳转到 JSP 做页面展示。
     * @param request
     * @param response
    ...
    private void doDetail(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        // 这个豆子只有一个，所以不需要袋子，只需要将这个咖啡豆放到 request 域当中即可。
        request.setAttribute("dept", dept);

        // 转发（不是重定向，因为要跳转到 JSP 做数据展示）
